...
describe('toPlainTime', () => {
  ...
  describe('should convert DayJS to Temporal.PlainTime', () => {
    ...
    it('converts a UTC DayJS preserving time components', () => {
      const value = dayJS.utc('2024-06-15T09:30:45.123Z');
      const result = action['~run']({ typed: true, value }, {});
      expect(result.typed).toBe(true);
      expect(result.value).toEqual(Temporal.PlainTime.from('09:30:45.123'));
    });

    it('converts a timezone-aware DayJS (CDT, UTC-5) — uses local time', () => {
    ...
    it('converts a DayJS at midnight (00:00:00)', () => {
      const value = dayJS.utc('2024-01-01T00:00:00.000Z');
      const result = action['~run']({ typed: true, value }, {});
      expect(result.typed).toBe(true);
      expect(result.value).toEqual(Temporal.PlainTime.from('00:00:00'));
    });

    it('converts a DayJS at end of day (23:59:59.999)', () => {
    ...
    it('converts DayJS during DST spring-forward (just before 02:00 CST, at 01:59:59)', () => {
    ...
    it('converts DayJS in Asia/Tokyo (UTC+9) — uses local time', () => {
