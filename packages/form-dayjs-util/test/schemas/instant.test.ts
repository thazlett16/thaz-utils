...
const aValidDayJS = dayJS('2024-06-15T12:00:00Z');
...
describe('instant', () => {
  describe('nullable overload', () => {
    ...
    test('converts a valid DayJS instance to Temporal.Instant', () => {
      const result = v.safeParse(schema, aValidDayJS);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.Instant.from(aValidDayJS.toISOString()));
    });
