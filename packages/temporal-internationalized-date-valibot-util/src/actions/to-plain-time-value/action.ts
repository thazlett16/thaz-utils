        ...
        '~run'(dataset, config) {
            try {
                if (dataset.value instanceof ZonedDateTime) {
                    dataset.value = Temporal.ZonedDateTime.from(dataset.value.toString()).toPlainTime();
                } else if (dataset.value instanceof CalendarDateTime) {
                    dataset.value = Temporal.PlainDateTime.from(dataset.value.toString()).toPlainTime();
                } else if (dataset.value instanceof Time) {
                    dataset.value = Temporal.PlainTime.from(dataset.value.toString());
