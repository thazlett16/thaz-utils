...
describe('toInstant', () => {
  ...
  describe('should convert @internationalized/date ZonedDateTime to Temporal.Instant', () => {
    ...
    it('converts a ZonedDateTime during DST fall-back (second 01:30, CST, UTC-6)', () => {
      const value = parseZonedDateTime('2024-11-03T01:30:00-06:00[America/Chicago]');
      const result = action['~run']({ typed: true, value }, {});
      expect(result.typed).toBe(true);
      // 01:30 CST (-6) = 07:30 UTC
      expect(result.value).toEqual(Temporal.Instant.from('2024-11-03T07:30:00.000Z'));
    });
    ...
    it('converts a ZonedDateTime in Asia/Tokyo (UTC+9)', () => {
      ...
      const result = action['~run']({ typed: true, value }, {});
      expect(result.typed).toBe(true);
      expect(result.value).toEqual(Temporal.Instant.from('2024-06-15T00:00:00.000Z'));
    });

    it('converts a ZonedDateTime in Australia/Sydney (AEDT, UTC+11)', () => {
    ...
    it('converts a ZonedDateTime using ZonedDateTime constructor', () => {
