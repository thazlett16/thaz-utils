...
describe('plainDate', () => {
  describe('nullable overload', () => {
    ...
    it('converts an @internationalized/date ZonedDateTime to Temporal.PlainDate', () => {
      const result = v.safeParse(schema, anIntlZdt);
      expect(result.success).toBeTruthy();
      expect(result.output).toEqual(Temporal.ZonedDateTime.from(anIntlZdt.toString()).toPlainDate());
    });

    it('converts an @internationalized/date CalendarDateTime to Temporal.PlainDate', () => {
      const result = v.safeParse(schema, aCalendarDateTime);
      expect(result.success).toBeTruthy();
      expect(result.output).toEqual(Temporal.PlainDateTime.from(aCalendarDateTime.toString()).toPlainDate());
    });

    it('converts an @internationalized/date CalendarDate to Temporal.PlainDate', () => {
      const result = v.safeParse(schema, aCalendarDate);
      expect(result.success).toBeTruthy();
      expect(result.output).toEqual(Temporal.PlainDate.from(aCalendarDate.toString()));
    });
