...
describe('plainDate', () => {
  describe('nullable overload', () => {
    ...
    test('converts an @internationalized/date ZonedDateTime to Temporal.PlainDate', () => {
      const result = v.safeParse(schema, anIntlZdt);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.ZonedDateTime.from(anIntlZdt.toString()).toPlainDate());
    });

    test('converts an @internationalized/date CalendarDateTime to Temporal.PlainDate', () => {
      const result = v.safeParse(schema, aCalendarDateTime);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.PlainDateTime.from(aCalendarDateTime.toString()).toPlainDate());
    });

    test('converts an @internationalized/date CalendarDate to Temporal.PlainDate', () => {
      const result = v.safeParse(schema, aCalendarDate);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.PlainDate.from(aCalendarDate.toString()));
    });
