...
describe('instant', () => {
  describe('nullable overload', () => {
    ...
    test('converts an @internationalized/date ZonedDateTime to Temporal.Instant', () => {
      const result = v.safeParse(schema, anIntlZdt);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.ZonedDateTime.from(anIntlZdt.toString()).toInstant());
    });
