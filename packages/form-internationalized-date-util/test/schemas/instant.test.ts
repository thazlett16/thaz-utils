...
describe('instant', () => {
  describe('nullable overload', () => {
    ...
    it('converts an @internationalized/date ZonedDateTime to Temporal.Instant', () => {
      const result = v.safeParse(schema, anIntlZdt);
      expect(result.success).toBeTruthy();
      expect(result.output).toEqual(Temporal.ZonedDateTime.from(anIntlZdt.toString()).toInstant());
    });
