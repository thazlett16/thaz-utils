...
describe('plainDateTime', () => {
  describe('nullable overload', () => {
    ...
    it('converts an @internationalized/date ZonedDateTime to Temporal.PlainDateTime', () => {
      const result = v.safeParse(schema, anIntlZdt);
      expect(result.success).toBeTruthy();
      expect(result.output).toEqual(Temporal.ZonedDateTime.from(anIntlZdt.toString()).toPlainDateTime());
    });

    it('converts an @internationalized/date CalendarDateTime to Temporal.PlainDateTime', () => {
      const result = v.safeParse(schema, aCalendarDateTime);
      expect(result.success).toBeTruthy();
      expect(result.output).toEqual(Temporal.PlainDateTime.from(aCalendarDateTime.toString()));
    });
