...
const anIntlZdt = parseZonedDateTime('2024-06-15T12:30:00+00:00[UTC]');
...
describe('plainDateTime', () => {
  describe('nullable overload', () => {
    ...
    test('converts an @internationalized/date ZonedDateTime to Temporal.PlainDateTime', () => {
      const result = v.safeParse(schema, anIntlZdt);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.ZonedDateTime.from(anIntlZdt.toString()).toPlainDateTime());
    });

    test('converts an @internationalized/date CalendarDateTime to Temporal.PlainDateTime', () => {
      const result = v.safeParse(schema, aCalendarDateTime);
      expect(result.success).toBeTruthy();
      expect(result.output).toStrictEqual(Temporal.PlainDateTime.from(aCalendarDateTime.toString()));
    });
